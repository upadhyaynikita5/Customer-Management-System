...
<body>
    ...
    <div >
    <h1 class="pt1 pb1">Customer Order</h1>
        <div class="admintable">
        <?php
            require("./database.php");

            $query = "SELECT * FROM orders where takeaway = 0";
            $result = $db_connection->query($query);

            if ($result) {
                if ($result->num_rows > 0) {
                    echo "<table  class='orders-table' border='1'>";
                    echo "<tr class='table-head'><th>Order ID</th><th>Table</th><th>Item</th><th>Name</th><th>Contact</th><th>Special Request</th></tr>";
